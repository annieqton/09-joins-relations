...
app.put('/articles/update', function(request, response) {
  ...
  function queryTwo(author_id) {
    client.query(
      `UPDATE authors SET author=$1, "authorUrl"=$2 WHERE author_id=$3`, // TODO: Write a SQL query to update an existing author record
  ...
  function queryThree(author_id) {
    client.query(
      `UPDATE articles SET title=$1, category=$2, "publishedOn"=$3, body=$4 WHERE article_id=$5`, // TODO: Write a SQL query to update an existing article record
      [request.body.title, request.body.category, request.body.publishedOn, request.body.body, request.body.id] // TODO: Add the values for this table as data for the SQL query
    );
  }
